...
@Component
public class FixMessageHandler2 {
    ...
    private Map<String, String> parseFixLineQuickFix(String line) {
        ...
        try {
            ...
            // fallback para split manual se não for possível parsear
    ...
    public void processFixFileRange(String inputPath, String outputCsvPath, int start, int end) throws IOException {
        ...
        synchronized (FixMessageHandler2.class) {
            if (start == 0) {
    ...
    public void processFullFillsRange(String inputPath, String outputPath, int start, int end) throws IOException {
        List<String> allLines = Files.readAllLines(Paths.get(inputPath));
        List<String> linesToWrite = new ArrayList<>();
        for (int i = start; i < end && i < allLines.size(); i++) {
            String line = allLines.get(i);
